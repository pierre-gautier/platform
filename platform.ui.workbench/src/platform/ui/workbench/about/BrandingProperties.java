...
public class BrandingProperties {
     ...
     * @return
     */
    public static ImageDescriptor getImage(final String value, final Bundle definingBundle) {
        final URL url = BrandingProperties.getUrl(value, definingBundle);
     ...
     *         <code>null</code>
     */
    public static ImageDescriptor[] getImages(final String value,
            final Bundle definingBundle) {
        final URL[] urls = BrandingProperties.getURLs(value, definingBundle);
        if (urls == null || urls.length <= 0) {
            return new ImageDescriptor[0];
        }
        
        final ImageDescriptor[] images = new ImageDescriptor[urls.length];
    ...
    public static URL getUrl(final String value, final Bundle definingBundle) {
        try {
            if (value != null) {
     ...
     */
    private static URL[] getURLs(final String value, final Bundle definingBundle) {
        if (value == null) {
            return new URL[0];
        }
        final StringTokenizer tokens = new StringTokenizer(value, ","); //$NON-NLS-1$
        final List<URL> array = new ArrayList<>(10);
