...
import platform.jetter.model.NodeDtoMapper;
...
public class NodeDtoClient
    ...
    public INode getById(final String id) {
        ...
        try {
            return this.mapper.toModel(this.client.target(this.url).path("id").path(id).request().get(NodeDto.class));
    ...
    @Override
    public void put(final Collection<INode> nodes) {
        if (CollectionsUtils.isNullOrEmpty(nodes)) {
            return;
        }
        final NodeDto[] array = new NodeDto[nodes.size()];
        ...
        final Builder builder = this.client.target(this.url).request();
        if (this.async) {
            builder.async().put(Entity.json(array));
        } else {
            builder.put(Entity.json(array));
        }
    }
